...
#warning This query needs to order results by relevance, but for now it just returns the first 20 candidates that match the criteria. The ordering can be improved later.
        ...
        var numberOfGeneratedCandidates = await _context.Database.SqlQueryRaw<NumberOfGeneratedCandidates>($"""
            select generate_candidates('{userId}') "Value"
        """).SingleAsync(cancellationToken);

        return numberOfGeneratedCandidates.Value > 0;

    }

    public record NumberOfGeneratedCandidates
    (
        int Value
    );
}